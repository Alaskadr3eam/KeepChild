...
//  KeepChild
//
//  Created by Clément Martin on 27/09/2019.
...
class HomeChatViewController: UIViewController {
 ...
 /*   @IBOutlet var tblUserList: UITableView!
    ...
    let arrConversationList = NSMutableArray()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Home"
        self.tblUserList.separatorStyle = .none
        let button1 = UIBarButtonItem(image: UIImage(named: "SideMenu"), style: .plain, target: self, action: #selector(HomeChatViewController.addTapped))
...
extension HomeChatViewController: UITableViewDataSource,UITableViewDelegate{
